...
import Seetickets from '../components/Seetickets'
...
useEffect (() => { 
  ...
  return (
    <div>
        <div className= {`ticket-page ${theme}`}> 
        <img onClick={() => toggleTheme()} id = 'switch' src={icon} /> 
          <Seetickets games = {games} rate = {rate} theme = {theme} seatmap = {games[0].seatmap.staticUrl}/> 
        </div>
    </div>
  )
}